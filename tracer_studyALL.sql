...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `ts_DeleteDaftarUrutanData` (IN `Pid` INT, IN `Pnama` VARCHAR(50), IN `Ptanggal_sekarang` DATETIME)  BEGIN
	UPDATE ts_daftarurutandata 
			...
			SELECT
			  GROUP_CONCAT(DISTINCT
			    CONCAT(
			      'MAX(IF(kode = ''',
			      kode,
			      ''', jawabanKuesioner, NULL)) AS ',
			      kode
			    )
			  ) INTO @sql
			FROM ts_laporankuesioner;

			SET @sql = CONCAT('SELECT id_hasilKuesioner, ', @sql, '\r\n\t\t\t                  FROM ts_laporankuesioner \r\n\t\t\t                  GROUP BY id_hasilKuesioner');
